...
if(!defined('ABSPATH'))
{
	...
	$folder = str_replace("/wp-content/plugins/mf_rentals/include/api", "/", dirname(__FILE__));
...
$arr_fields_excluded = array($obj_rentals->meta_prefix.'searchable');
...
switch($type_switch)
{
	case 'admin':
		if(is_user_logged_in())
		{
			...
			{
				$obj_rentals->option_type = "_".$arr_type[2];
			...
			switch($type_temp)
			{
				case 'webshop/list':
					...
					$result = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title, post_status, post_modified FROM ".$wpdb->posts." WHERE post_type = %s AND (post_status = %s OR post_status = %s)".$query_where, $obj_rentals->post_type_products.$obj_rentals->option_type, 'publish', 'draft'));
				...
				case 'webshop/edit':
					...
					if($post_id > 0)
					{
						$obj_rentals->get_option_type_from_post_id($post_id);

						$arr_meta_boxes = $obj_rentals->rwmb_meta_boxes(array());
						...
						$result = $wpdb->get_results($wpdb->prepare("SELECT post_title, post_name, post_type, post_author FROM ".$wpdb->posts." WHERE post_type = %s AND (post_status = %s OR post_status = %s) AND ID = '%d'".$query_where, $obj_rentals->post_type_products.$obj_rentals->option_type, 'publish', 'draft', $post_id));

						foreach($result as $r)
						{
							$setting_webshop_title_fields_amount = get_option_or_default('setting_webshop_title_fields_amount'.$obj_rentals->option_type, 1);
										...
										if($display_temp)
										{
											...
											switch($type_temp)
											{
												case 'custom_categories':
													$post_name_temp = str_replace($obj_rentals->meta_prefix, "", $id_temp);
													$post_id_temp = $wpdb->get_var($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE post_type = %s AND post_name = %s", $obj_rentals->post_type_document_type, $post_name_temp));

													$arr_data = array();
													get_post_children(array(
														'add_choose_here' => true,
														'post_type' => $obj_rentals->post_type_custom_categories,
														'join' => " INNER JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id AND meta_key = '".$obj_rentals->meta_prefix."document_type'",
											...
											if($value_temp == '' || $value_temp == 0)
											{
												switch($type_temp)
												{
													...
													case 'event':
														$obj_rentals->create_product_event_connection($post_id);
													break;
												}
											}
													...
													if(is_plugin_active("mf_calendar/index.php"))
													{
															...
															foreach($result_children as $r_children)
															{
																...
																if($event_end_date >= date("Y-m-d") || ($event_end_date < DEFAULT_DATE && $event_start_date >= date("Y-m-d") || $event_start_date < DEFAULT_DATE))
																{
																	...
																	$arr_children_temp[$r_children->ID] = array(
																		...
																		'fields' => $obj_rentals->get_event_fields(array('post_id' => $r_children->ID)),
																	);
																}
															}
														}
														...
														if(count($arr_children_temp) == 0)
														{
															$arr_children_temp[0] = array(
																...
																'end_date' => '',
																'end_time' => '',
																'fields' => $obj_rentals->get_event_fields(),
															);
														}
													}
												break;
				...
				case 'webshop/save':
					...
					if($post_id > 0)
					{
						...
						$result = $wpdb->get_results($wpdb->prepare("SELECT post_title, post_name, post_type FROM ".$wpdb->posts." WHERE post_type = %s AND (post_status = %s OR post_status = %s) AND ID = '%d'".$query_where, $obj_rentals->post_type_products.$obj_rentals->option_type, 'publish', 'draft', $post_id));
										...
										if(!in_array($id_temp, $arr_fields_excluded))
										{
											...
											switch($type_temp)
											{
																	...
																	if($arr_event_name[$i] != '')
																	{
																		...
																		else
																		{
																			...
																			if(wp_update_post($post_data_event) > 0)
																			{
																				...
																				$obj_rentals->set_events_meta_boxes($arr_event_id[$i], $i);
																...
																if($arr_event_name[$i] != '')
																{
																	...
																	else
																	{
																		...
																		if($post_id_temp > 0)
																		{
																			...
																			$obj_rentals->set_events_meta_boxes($post_id_temp, $i);
												...
												case 'file_advanced':
													...
													// Use this instead?
													/*if($obj_rentals->update_rwmb_post_meta($post_id, $id_temp, $arr_ids))
													{
														$updated = true;
													}*/
													#################################
												...
												case 'select3':
													...
													if($obj_rentals->update_rwmb_post_meta($post_id, $id_temp, $post_value_new))
													{
														$updated = true;
													}
												break;
					...
					{
						$obj_rentals->option_type = check_var('option_type');

						$result = $wpdb->get_results($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE post_type = %s AND post_status = %s AND post_title = %s AND post_author = '%d'", $obj_rentals->post_type_products.$obj_rentals->option_type, 'publish', $post_title, get_current_user_id()));

						if($wpdb->num_rows == 0)
						{
							$post_data = array(
								'post_title' => $post_title,
								'post_type' => $obj_rentals->post_type_products.$obj_rentals->option_type,
								'post_status' => 'publish',
							);
	...
	case 'calendar':
		...
		while($date_temp <= $date_end)
		{
			...
			$result = $obj_rentals->get_events(array('product_id' => $product_id, 'exact_date' => $date_temp, 'amount' => 5));
	...
	case 'events':
		...
		$json_output = $obj_rentals->get_events(array(
			'id' => $id,
	...
	case 'filter_products':
		...
		$json_output = $obj_rentals->get_filter_products(array(
			'id' => $id,
			'option_type' => $option_type,
			'category' => $category,
	...
	case 'amount':
	default:
		$obj_rentals->option_type = check_var('option_type', 'char');
		...
		/*$ghost_post_name = $obj_rentals->get_post_name_for_type('ghost');

		if($ghost_post_name != '')
		{
			$query_join .= " LEFT JOIN ".$wpdb->postmeta." AS meta_ghost ON ".$wpdb->posts.".ID = meta_ghost.post_id AND meta_ghost.meta_key = '".esc_sql($obj_rentals->meta_prefix.$ghost_post_name)."'";
			$query_order .= ($query_order != '' ? ", " : "")."meta_ghost.meta_value + 0 ASC";
		}*/

		$query_join .= " LEFT JOIN ".$wpdb->postmeta." AS searchable ON ".$wpdb->posts.".ID = searchable.post_id AND searchable.meta_key = '".$obj_rentals->meta_prefix.'searchable'."'";
		...
		switch($order)
		{
			...
			case 'size':
				$size_post_name = $obj_rentals->get_post_name_for_type('size');

				if($size_post_name != '')
				{
					$query_join .= " LEFT JOIN ".$wpdb->postmeta." AS meta_size ON ".$wpdb->posts.".ID = meta_size.post_id AND meta_size.meta_key = '".esc_sql($obj_rentals->meta_prefix.$size_post_name)."'";
		...
		$result = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title, post_excerpt, post_content".$query_select." FROM ".$wpdb->posts.$query_join." WHERE post_type = %s AND post_status = %s".$query_where.$query_group.$query_order, $obj_rentals->post_type_products.$obj_rentals->option_type, 'publish'));

		foreach($result as $r)
		{
			$obj_rentals->get_product_data(array('product' => $r, 'single_image' => true, 'show_location_in_data' => false), $json_output);
		}
