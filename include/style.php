...
if(!defined('ABSPATH'))
{
	...
	$folder = str_replace("/wp-content/plugins/mf_rentals/include", "/", dirname(__FILE__));
...
echo "@media all
{
	...
	if(!is_plugin_active("mf_widget_logic_select/index.php") || apply_filters('get_widget_search', 'webshop-events-widget') > 0 || apply_filters('get_widget_search', 'webshop-filter-products-widget') > 0)
	{
		...
		.webshop_filter_products .list_item, .webshop_events .list_item
		{
			...
			$result = $obj_rentals->get_category_colors(array('type' => 'category_background_color'));
	...
	if(is_plugin_active("mf_calendar/index.php"))
	{
		...
		if(!is_plugin_active("mf_widget_logic_select/index.php") || apply_filters('get_widget_search', 'webshop-events-widget') > 0)
		{
			...
			$result = $obj_rentals->get_category_colors();
						...
						.product_image_container a
						{
							...
							.is_mobile .product_image_container .category_icon
							{
								...
								foreach($obj_rentals->arr_option_types as $option_type)
								{
									...
									$custom_categories = $obj_rentals->get_post_name_for_type('custom_categories');

									if($custom_categories != '')
									{
										$result = $wpdb->get_results($wpdb->prepare("SELECT ID, meta_value FROM ".$wpdb->posts." INNER JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id WHERE post_type = %s AND meta_key = %s AND meta_value != '' GROUP BY meta_value", $obj_rentals->post_type_products.$option_type, $obj_rentals->meta_prefix.$custom_categories));
										...
										if($wpdb->num_rows > 0)
										{
											...
											foreach($result as $r)
											{
												...
												{
													$custom_category_img = get_post_meta_file_src(array('post_id' => $custom_category_id, 'meta_key' => $obj_rentals->meta_prefix.'image', 'image_size' => 'thumbnail', 'single' => true));
