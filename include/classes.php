...
class mf_rentals
{
	...
	var $post_type_categories;
	var $post_type_products;
	var $post_type_custom_categories;
	var $post_type_document_type;
	var $post_type_location;
	var $post_type_customers;
	var $post_type_delivery_type;
	var $default_templatete;
	var $template_shortcodes;
	var $option_type;
	var $event_max_length;
	var $user_updated_notification_subject_placeholder;
	var $user_updated_notification_content_placeholder;
	var $arr_option_types;
	var $option_type_temp;
	var $obj_font_icons;
	var $product_meta;
	var $product_id;
	var $product_title;
	...
	var $meta_symbol;
	var $product_content;
	var $product_form_buy;
	var $slideshow_images;
	var $arr_product_quick;
	var $arr_product_property;
	var $product_location;
	var $product_data;
	var $product_clock;
	var $default_template;
	var $interval_range_min;
	var $interval_range_max;
	var $template_used;
	var $footer_output;
	...
	function combined_head()
	{
		...
		{
			wp_enqueue_script('script_gmaps_api', "//maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&key=".$setting_gmaps_api, array());
		}
	...
	function admin_init()
	{
		...
		switch($pagenow)
		{
			...
			case 'admin.php':
				if(check_var('page') == 'mf_rentals/stats/index.php')
				{
					...
					<form method='post' action='#' class='mf_form' data-action='admin/webshop/save'>";
						...
						switch($setting_webshop_title_fields_amount)
						{
							...
							case 2:
								...
								$templates .= "<div".apply_filters('get_flex_flow', "").">"
	...
	function get_webshop_map($data = array())
	{
		...
		$out = "<div class='form_button webshop_map".($data['container_class'] != '' ? " ".$data['container_class'] : '')."'>";
			...
			$out .= "<div class='map_wrapper'>";
				...
				.input_hidden(array('name' => 'webshop_map_coordinates', 'allow_empty' => true))
		...
		$out = "<div id='webshop_search'>";
			...
			if($setting_webshop_display_filter != 'no')
			{
				if($setting_webshop_display_filter == 'button')
				{
					...
					$obj_rentals_interval->set_interval_amount($result);
					...
					foreach($result as $r)
					{
						...
						switch($post_custom_type)
						{
							...
							case 'interval':
								$obj_rentals_interval->increase_count();

								$post_title = get_post_meta_or_default($post_id, $this->meta_prefix.'document_alt_text', true, $post_title);

								$obj_rentals_interval->add_interval_type($post_name, $post_title);
								...
								foreach($result as $r)
								{
									...
									$obj_rentals_interval->set_range($post_meta_min);
								}

								$has_equal_amount = $obj_rentals_interval->has_equal_amount($post_title, $name_choose_here);
	...
	function render_slides($data)
	{
		...
		if(isset($obj_slideshow))
		{
			...
			$attributes['arr_images'] = $data['images'];

			$out .= $obj_slideshow->block_render_callback($attributes);
		}
	...
	function get_form_url($form_id)
	{
		...
		if($form_id > 0)
		{
			...
			/*foreach($this->get_pages_from_shortcode("[mf_form id=".$form_id."]") as $post_id)
			{
				$out = get_permalink($post_id);
			}*/
		}
	...
	function get_single_info($post)
	{
		...
		{
			$this->template_shortcodes['slideshow']['html'] = $this->render_slides(array(
				'images' => $this->slideshow_images,
				'settings' => array(
					'slideshow_style' => 'original',
					'slideshow_image_fit' => 'cover',
		...
		if($this->product_map != '')
		{
			...
			$this->template_shortcodes['map']['html'] = "<div class='form_button'>
				...
				$this->template_shortcodes['map']['html'] .= "</div>
			</div>";*/

			$this->combined_head();
			$this->get_map_resources();

			$this->template_shortcodes['map']['html'] = do_shortcode('[mf_maps]');

			$product_xtra = "class='map_coordinates' data-name='".$this->product_title."'";
			...
			$this->template_shortcodes['map']['html'] .= input_hidden(array(
					...
					if($this->product_has_email)
					{
						...
						if($setting_quote_form > 0)
						{
							$post_meta = "<div class='form_button'>
								<a href='".$this->get_form_url($setting_quote_form)."?products=".$this->product_id."' class='button'>".$this->meta_title."</a>
							</div>";
						}
					}
				break;
...
if(class_exists('mf_import'))
{
	class mf_webshop_import extends mf_import
	{
		function get_defaults()
		{
			...
			$this->obj_webshop = new mf_rentals();
...
if(class_exists('RWMB_Field') && class_exists('RWMB_Text_Field'))
{
	...
	class RWMB_Custom_Categories_Field extends RWMB_Text_Field
	{
		static public function html($meta, $field)
		{
			...
			$post_name = str_replace($obj_rentals->meta_prefix, "", $field['id']);
			$post_id = $wpdb->get_var($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE post_type = %s AND post_name = %s", $obj_rentals->post_type_document_type, $post_name));
...
class widget_webshop_search extends WP_Widget
{
	...
	function widget($args, $instance)
	{
		...
		echo apply_filters('filter_before_widget', $before_widget);
			...
			echo "<form action='".$this->obj_webshop->get_form_url(get_option('setting_quote_form'.$this->obj_webshop->option_type))."' method='post' id='product_form' class='mf_form product_search webshop_option_type".$this->obj_webshop->option_type."'>"
		...
		foreach($result as $r)
		{
			...
			switch($post_custom_type)
			{
				...
				case 'interval':
					...
					$out = $obj_rentals_interval->has_equal_amount($post_title, $name_choose_here);
				break;
			...
			if(count($instance['webshop_doc_type']) > 0)
			{
				...
				echo apply_filters('filter_before_widget', $before_widget);
					...
					echo "<form method='get' action='".get_permalink($instance['webshop_action'])."' class='mf_form'>
						<ul".apply_filters('get_flex_flow', "").">";
						...
						case 'slideshow':
							...
							if($html == '')
							{
								...
								if(is_array($arr_product_image) && count($arr_product_image) > 0)
								{
									...
									$html = $this->obj_webshop->render_slides(array(
										'images' => $arr_product_image,
										'settings' => array(
											'slideshow_style' => 'original',
											'slideshow_image_fit' => 'cover',
