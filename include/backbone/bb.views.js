...
var WebshopView = Backbone.View.extend(
{
	...
	initialize: function()
	{
		if(jQuery(".webshop_map.display_on_mobile").length > 0)
		{
			...
			{
				jQuery(".webshop_map.hide_on_mobile .webshop_map_container").attr("id", "webshop_map_hide").parents(".map_wrapper").addClass('hide');
			}
		...
		if(jQuery(".product_aside .widget.maps_map").length > 0)
		{
			...
			jQuery(function($)
			{
				self.show_map_coordinates({'fit_icons': true, 'remove_markers': true});
			});
		}
	...
	{
		if(typeof search_map_obj !== 'undefined' && search_map_obj !== null && search_map_obj && search_map_obj.length > 0 && search_map_obj.is(":visible"))
		{
