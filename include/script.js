...
{
	if(typeof markers !== 'undefined' && markers !== null && markers)
	{
		for(i in markers)
		{
...
function add_map_location(data)
{
	...
	{
		var pos = coordinates_temp.replace("(", "").replace(")", "").split(", "),
		...
		add_marker({'pos': pos, 'icon': data.icon, 'id': id, 'name': name, 'text': text});
	}
}
...
function fitIcons()
{
	...
	else if(typeof markers !== 'undefined' && markers !== null)
	{
		if(markers.length > 0)
		{
...
function init_maps()
{
	...
	if(has_maps == true && typeof init_map_object === 'function')
	{
	...
	if(has_maps == true)
	{
		...
		else if(typeof map_object !== 'undefined' && map_object !== null)
		{
...
jQuery(function($)
{
	...
	search_coordinates_obj = $(".map_coordinates");
