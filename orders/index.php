...
function update_product_amount($intProductID2, $intProductAmount2)
{
	...
	$result = $obj_rentals->get_document_types(array('select' => "ID, post_name", 'where_key' => "ID = '%d'", 'where_value' => $intProductID2, 'order' => "menu_order ASC"));
	...
	foreach($result as $r)
	{
		...
		$post_custom_type = get_post_meta($post_id, $obj_rentals->meta_prefix.'document_type', true);

		if($post_custom_type == 'price')
		{
			$post_meta = get_post_meta($intProductID2, $obj_rentals->meta_prefix.$post_name, true);

			if($post_meta > 0 && $intProductAmount2 > 0)
			{
				$intProductAmount_result = ($post_meta - $intProductAmount2 > 0 ? $post_meta - $intProductAmount2 : 0);

				update_post_meta($intProductID2, $obj_rentals->meta_prefix.$post_name, $intProductAmount_result);
			}
	...
	."<table class='widefat striped'>";
		...
		."<tbody>";
			...
			foreach($result as $r)
			{
				...
				$strCustomerName = $wpdb->get_var($wpdb->prepare("SELECT post_title FROM ".$wpdb->posts." WHERE post_type = %s AND post_status = %s AND ID = '%d'", $obj_rentals->post_type_customers, 'publish', $intCustomerID));
				...
				echo "<tr>
					...
					echo "</td>
					<td>
						<a href='?post_type=".$obj_rentals->post_type_products."&page=mf_rentals/orders/index.php&btnOrderShow&intOrderID=".$intOrderID2."'>".$strCustomerName."</a>
						<div class='row-actions'>";
					...
					echo "<tr".($class != '' ? " class='".$class."'" : "").">
						...
						<td colspan='".(count($arr_header) - 2)."'>
							<table>";
								...
								$result2 = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title, productAmount FROM ".$wpdb->posts." INNER JOIN ".$wpdb->prefix."webshop_product2user ON ".$wpdb->posts.".ID = ".$wpdb->prefix."webshop_product2user.productID WHERE post_type = %s AND orderID = '%d'", $obj_rentals->post_type_products, $intOrderID2));
