				...
				<div class='postbox'>
					...
					<div class='inside'>
						<ul>";
							...
							$location_post_name = $obj_rentals->get_post_name_for_type('location');

							$result = $wpdb->get_results($wpdb->prepare("SELECT locations.post_title, COUNT(products_meta.meta_value) AS locationAmount FROM ".$wpdb->base_prefix."form2answer INNER JOIN ".$wpdb->prefix."webshop_sent USING (answerID) INNER JOIN ".$wpdb->posts." AS products ON ".$wpdb->prefix."webshop_sent.productID = products.ID AND products.post_type = %s INNER JOIN ".$wpdb->postmeta." AS products_meta ON products.ID = products_meta.post_id AND products_meta.meta_key = '".$obj_rentals->meta_prefix.$location_post_name."' INNER JOIN ".$wpdb->posts." AS locations ON products_meta.meta_value = locations.ID WHERE answerCreated BETWEEN %s AND %s GROUP BY products_meta.meta_value ORDER BY locationAmount DESC".($intStatsLimit > 0 ? " LIMIT 0, ".$intStatsLimit : ""), $obj_rentals->post_type_products, $dteStatsDateStart, $dteStatsDateEnd));
